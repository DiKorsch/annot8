...
def creation_date(file,
    ...
    if exif is None:
        ...
        try:
            ...
            fname = str(Path(file.path.path).name)
            logging.error(f"Could get creation date from {fname}, ignoring this file!")
            return None
    return ts
